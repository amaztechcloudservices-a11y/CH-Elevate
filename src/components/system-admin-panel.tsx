...
type SystemData = {
  ...
  notifications: { smtpConfigured: boolean; bookingMailStates: Record<string, number>; note: string };
...
export function SystemAdminPanel() {
  ...
  return <div className="system-admin">
    ...
    {data && <div className="system-grid">
      ...
      <section className="cms-card"><h2><Mail /> Notifications</h2><p><State ready={data.notifications.smtpConfigured}>{data.notifications.smtpConfigured ? "SMTP configured" : "SMTP needs attention"}</State></p><dl>{Object.entries(data.notifications.bookingMailStates).map(([state, value]) => <div key={state}><dt>{state.replaceAll("_", " ")}</dt><dd>{value}</dd></div>)}</dl><p className="system-note">{data.notifications.note}</p></section>
