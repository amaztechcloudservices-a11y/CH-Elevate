...
export function PortalActivationForm({ token }: { token: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); setBusy(true); setMessage("Creating your secure account…"); const email = String(form.get("email")); const password = String(form.get("password")); const result = await authClient.signUp.email({ name: String(form.get("name")), email, password }); if (result.error) { const signIn = await authClient.signIn.email({ email, password }); if (signIn.error) { setBusy(false); setMessage(result.error.message || "Account activation failed. If you already have an account, enter its password."); return; } } const accepted = await fetch("/api/portal/invitations/accept", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token }) }); const body = await accepted.json() as { error?: string }; if (!accepted.ok) { setBusy(false); setMessage(body.error || "Invitation could not be accepted."); return; } window.location.assign("/portal"); }
  return <form className="portal-auth__form" method="post" onSubmit={submit}><label><span>Full name</span><input name="name" required /></label><label><span>Invited email address</span><input name="email" type="email" required /></label><label><span>Create or enter password</span><input name="password" type="password" minLength={8} required /></label><button type="submit" disabled={busy}>{busy ? <LoaderCircle className="spin" aria-hidden="true" /> : <LockKeyhole aria-hidden="true" />} Activate portal</button><p role="status">{message}</p></form>;
}
