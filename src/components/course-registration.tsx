...
export function CourseRegistration({ pageSlug }: { pageSlug: CourseCatalogueSectionInput["pageSlug"] }) {
  ...
  const [applicationType, setApplicationType] = useState<"individual" | "organisation">("individual");
  ...
  function useApplicantAsParticipant() {
    const form = dialogRef.current?.querySelector("form");
    if (!form) return;
    const values = new FormData(form);
    setParticipants((current) => [{ name: String(values.get("applicantName") || ""), email: String(values.get("applicantEmail") || ""), phone: String(values.get("applicantPhone") || "") }, ...current.slice(1)]);
  }
  ...
  return (
    <section className="course-registration" id="available-courses" aria-label="Published course catalogue" data-background={catalogue.section.backgroundType} data-tone={darkBackground ? "dark" : "light"} style={sectionStyle}>
      <div className="ref-container">
        ...
        <CoursePublicCatalogue cards={catalogue.data} offerings={offerings} onRegister={(offering) => { setSelected(offering); setParticipants([{ name: "", email: "", phone: "" }]); setApplicationType("individual"); setStatus({ kind: "idle", message: "" }); }} />
      </div>
      {selected && <dialog ref={dialogRef} className="course-application course-application--native" aria-labelledby="course-application-title" onCancel={(event) => { if (status.kind === "submitting") event.preventDefault(); else setSelected(null); }}><form onSubmit={submit}><header><div><span>Registration application</span><h2 id="course-application-title">{selected.title}</h2><p>{new Date(selected.startsAt).toLocaleDateString("en-JM", { dateStyle: "long", timeZone: selected.timeZone })}</p></div><button type="button" onClick={() => setSelected(null)} disabled={status.kind === "submitting"} aria-label="Close registration">×</button></header><div className="course-application__body">
        <fieldset className="course-application-type" disabled={status.kind === "submitting" || status.kind === "success"}><legend>Application type</legend><div role="radiogroup" aria-label="Application type"><label><input type="radio" name="applicationType" checked={applicationType === "individual"} onChange={() => { setApplicationType("individual"); setParticipants((current) => [current[0]]); }} /> Individual</label><label><input type="radio" name="applicationType" checked={applicationType === "organisation"} onChange={() => setApplicationType("organisation")} /> Organisation or team</label></div></fieldset>
        <fieldset disabled={status.kind === "submitting" || status.kind === "success"}><legend>Primary contact</legend><div className="course-form-grid"><label><span>Name</span><input name="applicantName" autoComplete="name" required /></label><label><span>Email</span><input name="applicantEmail" type="email" autoComplete="email" required /></label><label><span>Phone</span><input name="applicantPhone" autoComplete="tel" /></label>{applicationType === "organisation" && <label><span>Organisation</span><input name="organisationName" autoComplete="organization" required /></label>}</div></fieldset>
        <fieldset disabled={status.kind === "submitting" || status.kind === "success"}><legend>Participants ({participants.length} of 100)</legend><button className="course-add-person" type="button" onClick={useApplicantAsParticipant}>I am attending</button>{participants.map((person, index) => <div className="course-participant-row" key={index}><label><span>Name</span><input value={person.name} onChange={(event) => setParticipants(participants.map((item, i) => i === index ? { ...item, name: event.target.value } : item))} required /></label><label><span>Email</span><input type="email" value={person.email} onChange={(event) => setParticipants(participants.map((item, i) => i === index ? { ...item, email: event.target.value } : item))} required /></label><label><span>Phone</span><input value={person.phone} onChange={(event) => setParticipants(participants.map((item, i) => i === index ? { ...item, phone: event.target.value } : item))} /></label>{participants.length > 1 && <button type="button" aria-label={`Remove participant ${index + 1}`} onClick={() => setParticipants(participants.filter((_, i) => i !== index))}><Trash2 aria-hidden="true" /></button>}</div>)}{applicationType === "organisation" && <button className="course-add-person" type="button" disabled={participants.length >= 100} onClick={() => setParticipants([...participants, { name: "", email: "", phone: "" }])}><Plus aria-hidden="true" /> Add participant</button>}</fieldset>
        <aside className="course-application-summary" aria-label="Registration summary"><strong>{participants.length} participant{participants.length === 1 ? "" : "s"}</strong><span>{selected.feeCents ? `${formatMoney(selected.feeCents, selected.currency)} each · ${formatMoney(selected.feeCents * participants.length, selected.currency)} total` : "No course fee"}</span><p>Applications are reviewed before places are confirmed. If payment is required, CH Elevate will provide offline payment instructions after review.</p></aside>
