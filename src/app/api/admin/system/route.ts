...
import { deliverOperationalMail } from "@/server/site-mail";
...
export async function GET(request: Request) {
  try {
    ...
    const now = new Date();
    const [settings, storage, staff, sessions, bookingMailStates, operationalMailStates, mailAttention, latestAudit] = await Promise.all([
    ...
    return Response.json({ ok: true, data: {
      ...
      notifications: { smtpConfigured, bookingMailStates: Object.fromEntries(bookingMailStates.map((row) => [row.state, row.value])), operationalMailStates: Object.fromEntries(operationalMailStates.map((row) => [row.state, row.value])), attention: mailAttention, note: "Website and course mail is persisted before delivery. Failed messages retry with backoff; uncertain sends require review to avoid duplicates." },
